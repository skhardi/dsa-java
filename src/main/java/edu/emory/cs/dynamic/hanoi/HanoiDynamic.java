...
public class HanoiDynamic extends Hanoi {
    ...
    public List<String> solve(int n, char source, char intermediate, char destination) {
        ...
        System.out.println("Dynamic " + n + ": " + solve(list, n, source, intermediate, destination, new HashMap<>(), 0));
        return list;
    }

    private int solve(List<String> list, int n, char source, char intermediate, char destination, Map<String, int[]> map, int calls) {
        if (n == 0) return calls;
        ...
        else calls = solve(list, n - 1, source, destination, intermediate, map, ++calls);
        ...
        else calls = solve(list, n - 1, intermediate, source, destination, map, ++calls);
